...
It is also possible to define multiple paths for various folders(such as `@componentscomponents`, `@componentshooks`, etc.), but using `@components*` works very well because it is short enough so there is no need to configure multiple paths and it differs from other dependency modules so there is no confusion in what comes from `node_modules` and what is our source folder. That means that anything in the `src` folder can be accessed via `@`, e.g some file that lives in `src/components/MyComponent` can be accessed using `@componentscomponents/MyComponents`.
